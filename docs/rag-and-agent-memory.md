...
async def recall(path, question, your_adapter):
    async with await HistoryStore.open(path, config=Config(cache_backend="sqlite")) as memory:
        provider = MemoizedProvider(your_adapter, memory.config, cache=memory.cache,
